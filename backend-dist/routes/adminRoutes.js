...
router.put('/users/:id', verifyAdmin, async (req, res) => {
    try {
        ...
        const updateData = { displayName, email, phoneNumber, role, isActive };
        if (role === 'driver') {
            updateData.vehicleInfo = vehicleInfo;
            updateData.isVerified = isVerified;
        }
        const user = await User_1.User.findByIdAndUpdate(req.params.id, updateData, { new: true });
        res.json(user);
    }
    catch (error) {
        res.status(400).json({ error: error.message });
...
router.get('/drivers', verifyAdmin, async (req, res) => {
    try {
        const drivers = await User_1.User.find({ role: 'driver' }).sort({ createdAt: -1 });
        res.json(drivers);
    }
    catch (error) {
        res.status(400).json({ error: error.message });
...
router.post('/verify-driver/:id', verifyAdmin, async (req, res) => {
    try {
        ...
        const driver = await User_1.User.findByIdAndUpdate(req.params.id, { isVerified }, { new: true });
        res.json(driver);
    }
    catch (error) {
        res.status(400).json({ error: error.message });
...
router.get('/config', verifyAdmin, async (req, res) => {
    try {
        let config = await SystemConfig_1.SystemConfig.findOne();
        if (!config) {
            config = new SystemConfig_1.SystemConfig();
            await config.save();
        }
        res.json(config);
    }
...
router.post('/config', verifyAdmin, async (req, res) => {
    try {
        const config = await SystemConfig_1.SystemConfig.findOneAndUpdate({}, req.body, { new: true, upsert: true });
        res.json(config);
    }
    catch (error) {
        res.status(400).json({ error: error.message });
...
router.get('/rides', verifyAdmin, async (req, res) => {
    try {
        const rides = await Ride_1.Ride.find().populate('userId driverId').sort({ createdAt: -1 });
        res.json(rides);
    }
    catch (error) {
        res.status(400).json({ error: error.message });
...
router.get('/stats', verifyAdmin, async (req, res) => {
    try {
        ...
        const totalDrivers = await User_1.User.countDocuments({ role: 'driver' });
        const rides = await Ride_1.Ride.find({ status: 'completed' });
        ...
        const activeRides = await Ride_1.Ride.countDocuments({
...
router.post('/seed-demo-drivers', verifyAdmin, async (req, res) => {
    try {
        ...
        for (const d of demoDrivers) {
            const existing = await User_1.User.findOne({ email: d.email });
            if (!existing) {
                const user = new User_1.User(d);
                await user.save();
            }
        }
...
router.delete('/demo-drivers', verifyAdmin, async (req, res) => {
    try {
        await User_1.User.deleteMany({ isDemo: true });
...
router.post('/toggle-demo-online/:id', verifyAdmin, async (req, res) => {
    try {
        const driver = await User_1.User.findOne({ _id: req.params.id, isDemo: true });
        if (!driver)
            return res.status(404).json({ error: 'Demo driver not found' });
